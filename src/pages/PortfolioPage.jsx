    ...
    <Layout>
      ...
      <section className='h-[40vh] sm:h-[45vh] md:h-[50vh] w-full relative flex items-center justify-center'>
        ...
        <div className='flex flex-col items-center gap-4 md:gap-6 text-primary px-4 sm:px-8'>
          <h1 className='text-2xl sm:text-3xl md:text-4xl xl:text-5xl font-semibold text-center'>{CONTENT.portfolio.title}</h1>
          <p className='text-sm sm:text-base md:text-lg font-semibold text-center z-10 max-w-7xl'>{CONTENT.portfolio.subtitle}</p>
        </div>
      </section>

      {/* Portfolio Showcase */}
      <section className='px-4 sm:px-8 md:px-16 lg:px-24 xl:px-36 py-12 sm:py-16 md:py-20 lg:py-24 flex flex-col gap-8 md:gap-10 lg:gap-12 items-center'>
        {/* Filters */}
          ...
          {categories.map(cat => {
            ...
            return (
              <button
                ...
                className={`px-4 sm:px-5 md:px-6 py-1.5 sm:py-2 rounded-full border-1 cursor-pointer transition text-xs sm:text-sm font-medium tracking-wide ${active ? 'bg-primary text-secondary border-primary' : 'bg-secondary text-primary border-primary hover:bg-primary hover:text-secondary'}`}
              >
                {cat.label}
              </button>
            )
        ...
        {/* Cards pattern rows */}
        <div className='flex flex-col gap-6 md:gap-8 items-center w-full max-w-[1600px]'>
          {rows.map((row, rIndex) => {
            ...
            return (
              <div key={rIndex} className='flex flex-col md:flex-row gap-6 md:gap-8 justify-center w-full'>
                {items.map(item => (
                  <PortfolioCard
                    key={item.id}
                    id={item.id}
          ...
          {filtered.length === 0 && (
            <div className='text-center text-primary/60 text-xs sm:text-sm py-10'>Tidak ada proyek.</div>
          )}
        </div>
      </section>
    </Layout>
